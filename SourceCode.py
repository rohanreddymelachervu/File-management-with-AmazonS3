    ...
    def decryption(self):  # produces the original result
        ...
        try:
            ...
            decrypted_file = input(
                'Enter the filename for the Decryption file with extension:')  # Decrypted file as output
...
def fernet_cipher_and_save_file(filename, fkey):
    f = Fernet(fkey)
...
def fernet_decipher_and_save_file(filename, fkey):
    f = Fernet(fkey)
    with open(filename, "rb") as file:
        # read the encrypted data
        encrypted_data = file.read()
...
"""
def check_key():
    my_db = mysql.connector.connect(host="localhost", user="root", passwd="Bruhan@123", database="os_project",
                                    autocommit=True)
    cur = my_db.cursor()
...
def get_symmetric_cipher_key(user_id):
    my_db = mysql.connector.connect(host="localhost", user="root", passwd="Bruhan@123", database="os_project",
                                    autocommit=True)
    cur = my_db.cursor()
    cur.execute("select key_code from file_sec where name = '" + user_id + "';")
    key = ''
    for i in cur:
        key = i[0]
    if len(key) < 1:
        ...
        # key_string = (key, 'utf-8')
        st = "UPDATE file_sec SET key_code = '" + key.decode() + "' WHERE name = '" + user_id + "';"
        cur.execute(st)
        my_db.commit()
    else:
        return key.encode()
...
if a == 2:
    ...
    my_db = mysql.connector.connect(host="localhost", user="root", passwd="Bruhan@123", database="os_project",
                                    autocommit=True)
    # my_db = mysql.connector.connect("localhost", "root", "Bruhan@123", "os_project", True)
    cur = my_db.cursor()
    ...
    if b == 3:
        ...
        #cur.execute(ss, rec)
        print("YOU ARE ALL SET!!KINDLY EXIT AND RERUN FOR LOGIN")
        q = int(input())
        if q == 4:
            os.system("exit")
    else:
    ...
    os.system("cls")
    print("IF U WANT TO SELECT A FILE FROM UR PC AND UPLOAD IT TO AWS S3 THEN PRESS 1")
    print("IF U WANT TO DOWNLOAD ENCRYPTED FILES FROM AWS S3 AND DECRYPT IT FOR VIEWING - THEN PRESS 2")
    fg = int(input())
    os.system("cls")
    if fg == 1:
        res = ''
        # call res= getSymmetricCipherKey(userName)
        """
        my_db = mysql.connector.connect(host="localhost", user="root", passwd="Bruhan@123", database="os_project",
                                        autocommit=True)
        cur = my_db.cursor()
        ...
        for i in cur:
            res = i[0]
        """
        res= get_symmetric_cipher_key(uname)
        print("WELCOME TO THE MAIN PAGE")
        print("PLEASE GIVE THE FILE NAME(WITH EXTENSION) WHICH YOU WANT TO STORE IN CLOUD: ")
        fname = input()
        nf1 = open(str(fname.split(".")[0]) + '1.' + str(fname.split(".")[1]), "wb")
        nf2 = open(str(fname.split(".")[0]) + '2.' + str(fname.split(".")[1]), "wb")
        nf3 = open(str(fname.split(".")[0]) + '3.' + str(fname.split(".")[1]), "wb")
        with open(fname, "r", encoding="utf8") as file:
            data = file.readlines()
            si = len(data)
            for i in range(si // 3):
                nf1.write(data[i].encode())
                # nf1.write('Why no work!.\n')
            for j in range(si // 3, 2 * si // 3):
                nf2.write(data[j].encode())
            for k in range(2 * si // 3, si):
                nf3.write(data[k].encode())
            nf1.close()
            nf2.close()
            nf3.close()

        # check_key()
        fernet_cipher_and_save_file(str(fname.split(".")[0]) + '1.' + str(fname.split(".")[1]), res)
        print("FILE 1st SUBPART HAS BEEN SUCCESSFULLY ENCRYPTED")
        fernet_cipher_and_save_file(str(fname.split(".")[0]) + '2.' + str(fname.split(".")[1]), res)
        print("FILE 2nd SUBPART HAS BEEN SUCCESSFULLY ENCRYPTED")
        fernet_cipher_and_save_file(str(fname.split(".")[0]) + '3.' + str(fname.split(".")[1]), res)
        print("FILE 3rd SUBPART HAS BEEN SUCCESSFULLY ENCRYPTED")
        s3.upload_file(str(fname.split(".")[0]) + '1FE.' + str(fname.split(".")[1]), 'os-project',
                       str(fname.split(".")[0]) + '1FE.' + str(fname.split(".")[1]))
        ...
        print("FILE HAS BEEN UPLOADED SECURELY USING 3SES. PLEASE PRESS 4 TO EXIT THE SYSTEM")
        h = int(input())
        if h == 4:
            os.system("exit")
        else:
        ...
        df1=s3.download_file('os-project', str(bname.split(".")[0]) + '1FE.' + str(bname.split(".")[1]),
                         str(bname.split(".")[0]) + '1DE.' + str(bname.split(".")[1]))
        print("FILE 1 HAS BEEN SUCCESSFULLY DOWNLOADED FROM AWS")
        df2=s3.download_file('os-project', str(bname.split(".")[0]) + '2FE.' + str(bname.split(".")[1]),
                         str(bname.split(".")[0]) + '2DE.' + str(bname.split(".")[1]))
        print("FILE 2 HAS BEEN SUCCESSFULLY DOWNLOADED FROM AWS")
        df3=s3.download_file('os-project', str(bname.split(".")[0]) + '3FE.' + str(bname.split(".")[1]),
        ...
        for i in cur:
            res1 = i[0]
        """
        res1 = get_symmetric_cipher_key(uname)
        fernet_decipher_and_save_file(str(bname.split(".")[0]) + '1DE.' + str(bname.split(".")[1]),res1)
                    #str("decrypt_" + bname.split(".")[0]) + '1.' + str(bname.split(".")[1]),res1)
        print("SUCCESSFULLY DECRYPTED FILE No 1!!!")
        # decrypt(str(bname.split(".")[0])+'2DE.'+str(bname.split(".")[1]),
        # str("decrypt_"+bname.split(".")[0])+'2.'+str(bname.split(".")[1]))
        fernet_decipher_and_save_file(str(bname.split(".")[0]) + '2DE.' + str(bname.split(".")[1]), res1)
        print("SUCCESSFULLY DECRYPTED FILE No 2!!!")
        fernet_decipher_and_save_file(str(bname.split(".")[0]) + '3DE.' + str(bname.split(".")[1]), res1)
                    #str("decrypt_" + bname.split(".")[0]) + '3.' + str(bname.split(".")[1]), res1)
        print("SUCCESSFULLY DECRYPTED FILE No 3!!!")
        merge_file(str("decrypt_" + bname.split(".")[0]) + '1DE.' + str(bname.split(".")[1]),
                   str("decrypt_" + bname.split(".")[0]) + '2DE.' + str(bname.split(".")[1]),
                   str("decrypt_" + bname.split(".")[0]) + '3DE.' + str(bname.split(".")[1]))
